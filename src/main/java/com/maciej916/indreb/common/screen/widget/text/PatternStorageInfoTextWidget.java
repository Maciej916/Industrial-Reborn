...
public class PatternStorageInfoTextWidget extends BaseWidget {
    ...
    public void renderButton(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        if (result.getResultStack() != ItemStack.EMPTY) {
            ...
            GuiUtil.renderScaled(poseStack, Component.translatable("gui." + IndReb.MODID + ".scanner.energy_cost").getString() + " " + TextComponentUtil.getFormattedStorageUnit(result.getEnergyCost(), true) + " IE", getX() + 3, getY() + 16, 0.75f, 0x00a200, false);
        } else {
            GuiUtil.renderScaled(poseStack, Component.translatable("gui." + IndReb.MODID + ".scanner.item").getString() + " -", getX() + 3, getY(), 0.75f, 0xd60000, false);
            GuiUtil.renderScaled(poseStack, Component.translatable("gui." + IndReb.MODID + ".scanner.matter_cost").getString() + " 0.00 mB", getX() + 3, getY() + 8, 0.75f, 0xd60000, false);
            GuiUtil.renderScaled(poseStack, Component.translatable("gui." + IndReb.MODID + ".scanner.energy_cost").getString() + " 0 IE/t", getX() + 3, getY() + 16, 0.75f, 0xd60000, false);
        }
