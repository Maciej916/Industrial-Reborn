...
public class BlockEntityThermalCentrifuge extends IndRebBlockEntity implements IHasExp, IHasUpgrades, IBlockEntityEnergy, IHasSound {
    ...
    public Progress progressTemp = new Progress(0, 3000);
    ...
    public BlockEntityThermalCentrifuge(BlockPos pos, BlockState blockState) {
        ...
        this.data = new ContainerData() {
            ...
            public int get(int index) {
                return switch (index) {
                    case 0 -> BlockEntityThermalCentrifuge.this.progressRecipe.getContainerDataCurrent();
                    case 1 -> BlockEntityThermalCentrifuge.this.progressRecipe.getContainerDataMax();
                    case 2 -> BlockEntityThermalCentrifuge.this.progressTemp.getContainerDataCurrent();
                    case 3 -> BlockEntityThermalCentrifuge.this.progressTemp.getContainerDataMax();
                    default -> 0;
                };
            }
            ...
            public void set(int index, int value) {
                switch (index) {
                    case 0 -> BlockEntityThermalCentrifuge.this.progressRecipe.setContainerDataCurrent(value);
                    case 1 -> BlockEntityThermalCentrifuge.this.progressRecipe.setContainerDataMax(value);
                    case 2 -> BlockEntityThermalCentrifuge.this.progressTemp.setContainerDataCurrent(value);
                    case 3 -> BlockEntityThermalCentrifuge.this.progressTemp.setContainerDataMax(value);
                }
            }
    ...
    public void tickWork() {
        ...
        if (recipe != null) {
            ...
            if (canWork()) {
                if (
                    ...
                    if (progressTemp.currentProgress() >= recipe.getTemperature()) {
                        progressRecipe.incProgress(1);
                    }

                    if (getTickCounter() == 20) {
                        if (progressTemp.currentProgress() + 1.5f <= progressTemp.getProgressMax()) {
        ...
        if (activeState) {
            ...
            if (progressTemp.currentProgress() > 0) {
                if (getTickCounter() == 20) {
                    progressTemp.decProgress(Math.min(progressTemp.currentProgress(), 2.5f));
                }
            }
        }
    }
    ...
    public AbstractContainerMenu createMenu(int containerId, Inventory playerInventory, Player player) {
        return new MenuThermalCentrifuge(this, containerId, playerInventory, player, data);
    }
    ...
    public List<BaseOneProbeInfo> addProbeInfo(List<BaseOneProbeInfo> oneProbeInfo) {
        ...
        oneProbeInfo.add(new ProbeInfoSimpleText(Component.translatable("gui." + IndReb.MODID + ".temperature", GuiUtil.DECIMAL_FORMAT_1.format(progressTemp.currentProgress()) + " " + GuiUtil.DEGREE_SYMBOL + "C")));
