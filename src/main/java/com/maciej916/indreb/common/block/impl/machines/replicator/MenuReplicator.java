...
public class MenuReplicator extends IndRebMenu {
    ...
    public MenuReplicator(int windowId, Inventory inv, FriendlyByteBuf data) {
        this(windowId, inv.player.getCommandSenderWorld(), data.readBlockPos(), inv, inv.player);
    }

    public MenuReplicator(int windowId, Level world, BlockPos pos, Inventory playerInventory, Player player) {
        super(ModMenuTypes.REPLICATOR.get(), windowId, world, pos, playerInventory, player);
