...
public class MenuExtruder extends IndRebMenu {
    ...
    public MenuExtruder(int windowId, Inventory inv, FriendlyByteBuf data) {
        this(windowId, inv.player.getCommandSenderWorld(), data.readBlockPos(), inv, inv.player);
    }

    public MenuExtruder(int windowId, Level world, BlockPos pos, Inventory playerInventory, Player player) {
        super(ModMenuTypes.EXTRUDER.get(), windowId, world, pos, playerInventory, player);
