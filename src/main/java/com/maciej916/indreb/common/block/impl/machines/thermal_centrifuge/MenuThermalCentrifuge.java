...
public class MenuThermalCentrifuge extends IndRebMenu {
    ...
    public MenuThermalCentrifuge(int windowId, Inventory inv, FriendlyByteBuf data) {
        this(windowId, inv.player.getCommandSenderWorld(), data.readBlockPos(), inv, inv.player);
    }

    public MenuThermalCentrifuge(int windowId, Level world, BlockPos pos, Inventory playerInventory, Player player) {
        super(ModMenuTypes.THERMAL_CENTRIFUGE.get(), windowId, world, pos, playerInventory, player);
