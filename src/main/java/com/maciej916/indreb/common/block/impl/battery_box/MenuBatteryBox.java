...
public class MenuBatteryBox extends IndRebMenu {
    ...
    public MenuBatteryBox(int windowId, Inventory inv, FriendlyByteBuf data) {
        this(windowId, inv.player.getCommandSenderWorld(), data.readBlockPos(), inv, inv.player);
    }

    public MenuBatteryBox(int windowId, Level world, BlockPos pos, Inventory playerInventory, Player player) {
        super(ModMenuTypes.BATTERY_BOX.get(), windowId, world, pos, playerInventory, player);
