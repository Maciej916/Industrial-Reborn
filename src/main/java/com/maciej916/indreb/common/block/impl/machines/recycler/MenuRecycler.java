...
public class MenuRecycler extends IndRebMenu {
    ...
    public MenuRecycler(int windowId, Inventory inv, FriendlyByteBuf data) {
        this(windowId, inv.player.getCommandSenderWorld(), data.readBlockPos(), inv, inv.player);
    }

    public MenuRecycler(int windowId, Level world, BlockPos pos, Inventory playerInventory, Player player) {
        super(ModMenuTypes.RECYCLER.get(), windowId, world, pos, playerInventory, player);
