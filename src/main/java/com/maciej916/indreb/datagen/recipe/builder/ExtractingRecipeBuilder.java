...
public class ExtractingRecipeBuilder {
    ...
    private ExtractingRecipeBuilder(ItemStack result, int duration, int tickEnergyCost, float experience) {
        this.result = result;
    ...
    public static ExtractingRecipeBuilder builder(RegistryObject<Item> item, int resultCount) {
        return builder(item.get(), resultCount);
    }

    public static ExtractingRecipeBuilder builder(ItemLike item, int resultCount) {
        return new ExtractingRecipeBuilder(new ItemStack(item, resultCount), 180, 8, 0);
    }

    private ExtractingRecipeBuilder setIngredient(Ingredient ingredient, int count) {
        this.ingredients.setIngredient(0, ingredient, count);
        return this;
    }

    public ExtractingRecipeBuilder setIngredient(RegistryObject<Item> item, int count) {
        return setIngredient(Ingredient.of(item.get()), count);
    }

    public ExtractingRecipeBuilder setIngredient(ItemLike item, int count) {
        return setIngredient(Ingredient.of(item), count);
    }

    public ExtractingRecipeBuilder setIngredient(TagKey<Item> tag, int count) {
        return setIngredient(Ingredient.of(tag), count);
    }

    public ExtractingRecipeBuilder setDuration(int duration) {
        this.duration = duration;
        return this;
    }

    public ExtractingRecipeBuilder setTickEnergyCost(int tickEnergyCost) {
        this.tickEnergyCost = tickEnergyCost;
        return this;
    }

    public ExtractingRecipeBuilder setExperience(float experience) {
        this.experience = experience;
        return this;
    }

    public ExtractingRecipeBuilder addChanceResult(ItemLike item, int count, int chance) {
        this.chanceResults = this.chanceResults.addChanceResult(new ItemStack(item, count), chance);
        return this;
    }

    public ExtractingRecipeBuilder addChanceResult(RegistryObject<Item> item, int count, int chance) {
        this.chanceResults = this.chanceResults.addChanceResult(new ItemStack(item.get(), count), chance);
        return this;
    }

    public ExtractingRecipeBuilder setGroup(String groupIn) {
        this.group = groupIn;
        return this;
    }

    public ExtractingRecipeBuilder addCriterion(String name, CriterionTriggerInstance criterionIn) {
    ...
    public static class Result implements FinishedRecipe {
        ...
        private final ExtractingRecipeBuilder builder;

        public Result(ResourceLocation id, ExtractingRecipeBuilder builder) {
            this.id = id;
