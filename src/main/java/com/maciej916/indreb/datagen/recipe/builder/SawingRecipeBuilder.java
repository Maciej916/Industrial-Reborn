...
public class SawingRecipeBuilder {
    ...
    private SawingRecipeBuilder(ItemStack result, int duration, int tickEnergyCost, float experience) {
        this.result = result;
    ...
    public static SawingRecipeBuilder builder(RegistryObject<Item> item, int resultCount) {
        return builder(item.get(), resultCount);
    }

    public static SawingRecipeBuilder builder(ItemLike item, int resultCount) {
        return new SawingRecipeBuilder(new ItemStack(item, resultCount), 180, 8, 0);
    }

    private SawingRecipeBuilder setIngredient(Ingredient ingredient, int count) {
        this.ingredients.setIngredient(0, ingredient, count);
        return this;
    }

    public SawingRecipeBuilder setIngredient(RegistryObject<Item> item, int count) {
        return setIngredient(Ingredient.of(item.get()), count);
    }

    public SawingRecipeBuilder setIngredient(ItemLike item, int count) {
        return setIngredient(Ingredient.of(item), count);
    }

    public SawingRecipeBuilder setIngredient(TagKey<Item> tag, int count) {
        return setIngredient(Ingredient.of(tag), count);
    }

    public SawingRecipeBuilder setDuration(int duration) {
        this.duration = duration;
        return this;
    }

    public SawingRecipeBuilder setTickEnergyCost(int tickEnergyCost) {
        this.tickEnergyCost = tickEnergyCost;
        return this;
    }

    public SawingRecipeBuilder setExperience(float experience) {
        this.experience = experience;
        return this;
    }

    public SawingRecipeBuilder addChanceResult(ItemLike item, int count, int chance) {
        this.chanceResults = this.chanceResults.addChanceResult(new ItemStack(item, count), chance);
        return this;
    }

    public SawingRecipeBuilder addChanceResult(RegistryObject<Item> item, int count, int chance) {
        this.chanceResults = this.chanceResults.addChanceResult(new ItemStack(item.get(), count), chance);
        return this;
    }

    public SawingRecipeBuilder setGroup(String groupIn) {
        this.group = groupIn;
        return this;
    }

    public SawingRecipeBuilder addCriterion(String name, CriterionTriggerInstance criterionIn) {
    ...
    public static class Result implements FinishedRecipe {
        ...
        private final SawingRecipeBuilder builder;

        public Result(ResourceLocation id, SawingRecipeBuilder builder) {
            this.id = id;
